	...
	setupImageSwitch: function() {
		...
		$(".catalog-list-items .product .media-list").on("mouseenter", function() {
			...
			if( list.length > 1 ) {
				...
				second.fadeTo(0, 0.80);

				list.first().fadeTo(400, 0.80, function() {
					list.css('top', -size);
					second.fadeTo(400, 1);
				});
			}
		...
		$(".catalog-list-items .product .media-list").on("mouseleave", function() {
			...
			if( list.length > 1 ) {
				list.first().css('opacity', 0.80);

				list.eq(1).fadeTo(400, 0.80, function() {
					list.css('top', 0);
...
jQuery(document).ready(function($) {
	...
	$(".stockitem .stocktext").text(function () {
		return $(this).text().replace("PRODUKT IST AUF LAGER STOCKTYPE:CA.", "Lieferzeit: ca."); 
	});
	/*****direct redirect*****/
