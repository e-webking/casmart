...
$lotsize = $current * $size;

if($lotsize > $total)
    $lotsize = $total;

$showing = ($total >= $size) ? $lotsize : $total;

if ($this->get('showbottom') == 0):

?>
<div class="armpagenumber">Zeige <?= $showing; ?> von <?= $total ?> <?= $enc->html( $this->get('category') ); ?></div>
<div class="armpagination">
    <div class="sort">
        Sortieren &nbsp; <a class="listtoggle" href="javascript:void(0);"><i class="fa fa-angle-down"></i></a>
        <br><ul class="armulist" style="display:none;">
            <li>
...
<?php endif; ?>
 ...
 <?php if ($this->get('showbottom') == 1): ?>
	...
	<?php if( $this->last > 1 ) : ?>
         ...
         <?php else: ?>
            <span><?= $enc->html( sprintf( $this->translate( 'client', 'Seite %1$d von %2$d' ), $this->current, $this->last ) ); ?></span>
	<?php endif; ?>
    </div>
<?php endif; ?>
